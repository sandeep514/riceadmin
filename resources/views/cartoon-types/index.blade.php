                                        ...
                                        @foreach($records as $record)
                                            <tr>
                                                <td data-order="{{ $record->order_no ?? 999999 }}">
                                                    @include('components.master-order-input', ['model' => $record, 'route' => 'update.order.cartoon-type'])
                                                </td>
                                                <td>{{ $record->type }}</td>
                                                <td>{{ $record->description ?: '—' }}</td>
                                                <td>
                                                    @if((int) $record->status === \App\CartoonType::STATUS_ACTIVE)
                                                ...
                                                <td>
                                                    @include('cartoon-types._actions', ['model' => $record])
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
