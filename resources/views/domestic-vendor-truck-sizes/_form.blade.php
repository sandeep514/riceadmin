<div class="box-body">
    <div class="row">
        <div class="form-group col-md-3 @error('size') has-error @enderror">
            {!! Form::label('size', 'Size (MT)*') !!}
            {!! Form::number('size', isset($model) ? $model->size : old('size'), ['class' => 'form-control', 'id' => 'size', 'min' => '0.01', 'step' => '0.01', 'placeholder' => '16']) !!}
            @error('size')<span class="help-block text-danger" role="alert">{{ $message }}</span>@enderror
        </div>
        <div class="form-group col-md-5 @error('label') has-error @enderror">
            {!! Form::label('label', 'Label') !!}
            {!! Form::text('label', isset($model) ? $model->label : old('label'), ['class' => 'form-control', 'id' => 'label', 'maxlength' => 255, 'placeholder' => '16 MT']) !!}
            @error('label')<span class="help-block text-danger" role="alert">{{ $message }}</span>@enderror
        </div>
