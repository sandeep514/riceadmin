<div class="box-body">
    <div class="row">
        <div class="form-group col-md-4 @error('country_id') has-error @enderror">
            ...
            {!! Form::select('country_id', ['' => 'Select country'] + ($countries ?? []), isset($model) ? $model->country_id : old('country_id'), ['class' => 'form-control', 'id' => 'country_id']) !!}
            @error('country_id')<span class="help-block text-danger" role="alert">{{ $message }}</span>@enderror
        </div>
        <div class="form-group col-md-4 @error('name') has-error @enderror">
            {!! Form::label('name', 'State*') !!}
            {!! Form::text('name', null, ['class' => 'form-control', 'id' => 'name', 'maxlength' => 255, 'placeholder' => 'Enter state']) !!}
