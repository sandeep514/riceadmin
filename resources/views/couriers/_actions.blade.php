...
@if(\App\Permission::hasPermission('edit'))
    <a href="{{ route('edit.courier',$model->id) }}" class="btn btn-info btn-xs">Edit</a>
    @php($actionText = false)
@endif
@if(\App\Permission::hasPermission('delete'))
    |
    {!! Form::open(['method'=>'DELETE','route'=>['delete.courier',$model->id],'class'=>'delete-form','style'=>'display: inline-block;']) !!}
